...
tax_assign <- read.csv("taxonomy_assignment.txt",
                     sep = "\t", dec = ".", header = T)
head(tax_assign)
...
# phylum
SRS_dna_phylum <- cbind(SRS_dna_taxa$phylum, SRS_dna_taxa[-c(100:ncol(SRS_dna_taxa))])
colnames(SRS_dna_phylum) <- c("phylum", colnames(SRS_dna_taxa[-c(100:ncol(SRS_dna_taxa))]))
SRS_dna_phylum

SRS_dna_phylum_collap <- otuCollap(SRS_dna_phylum, taxto = 1, taxhead = "phylum",
                                       pattern = "_____________")
colnames(SRS_dna_phylum_collap) <- colnames(SRS_dna_phylum)
SRS_dna_phylum_collap
dim(SRS_dna_phylum_collap)


## relative abundances of phyla were calculated for each type and imported
dna_phyla_relabund <- read.table("dna_phyla_relabund.txt", sep="\t", dec=".", header=TRUE)
dna_phyla_relabund
dim(dna_phyla_relabund)

dna_phyla_relabund$phylum <- ifelse(rowSums(dna_phyla_relabund[2:26]/ncol(dna_phyla_relabund[2:26])) < 1, "<1 % Phyla", as.character(dna_phyla_relabund$phylum)) # renaming all phyla under 1%
rare_phyla <- dna_phyla_relabund[which(dna_phyla_relabund$phylum == "<1 % Phyla"),] #show list of phyla <0.1%
rare_phyla <- colSums(rare_phyla[2:26]) #calculating total relative abundance of new "Rare" group per sample 
dna_phyla_relabund <- dna_phyla_relabund[-which(dna_phyla_relabund$phylum == "<1 % Phyla"),]
rare_phyla <- append(rare_phyla, "<1 % Phyla (34 other taxa)") #adding col names
names(rare_phyla) <- c("bremen.HDPE", "bremen.wood", "bremen.tw", "brake.HDPE", "brake.wood", "brake.tw", 
                       ...
                       "brake.water3", "brake.water02", "bremerhaven.water3", "bremerhaven.water02", "helgolandT.water3", "helgolandT.water02", "phylum")
rare_phyla <- rare_phyla[c(26,1:25)] #bring phylum name to first row
dna_phyla_relabund <- rbind(dna_phyla_relabund,as.data.frame(t(rare_phyla)))
dim(dna_phyla_relabund)
head(dna_phyla_relabund)

#make all numeric
for (i in 2:length(dna_phyla_relabund)) {
  dna_phyla_relabund[,i] <- as.numeric(dna_phyla_relabund[,i])
}

dna_phyla_relabund

#stretching palette
n_phyla <- nrow(dna_phyla_relabund)
palette_phyla <- colorRampPalette(RColorBrewer::brewer.pal(11, "RdYlBu"))(n_phyla)


# subsetting by type
####hdpe
phyla_hdpe <- dna_phyla_relabund[c("phylum", "bremen.HDPE", "brake.HDPE", "bremerhaven.HDPE", "helgolandT.HDPE", "helgolandS.HDPE")]
colnames(phyla_hdpe) <- c("Phylum", "Bremen", "Brake", "Bremerhaven", "Helgoland (T)", "Helgoland (S)")
phyla_hdpe
dim(phyla_hdpe)

#change to long format
phyla_hdpe_melt <- melt(phyla_hdpe)
phyla_hdpe_melt$Phylum <- factor(phyla_hdpe_melt$Phylum, levels = phyla_hdpe$Phylum) 

#define variables
phyla_hdpe_melt$sites <- plyr::mapvalues(phyla_hdpe_melt$variable, 
                                   c("Bremen", "Brake", "Bremerhaven", "Helgoland (T)", "Helgoland (S)"), 
                                   c("Bremen", "Brake", "Bremerhaven", "Helgoland (T)", "Helgoland (S)"))
phyla_hdpe_melt

####tyre wear
phyla_tw <- dna_phyla_relabund[c("phylum", "bremen.tw", "brake.tw", "bremerhaven.tw", "helgolandT.tw", "helgolandS.tw")]
colnames(phyla_tw) <- c("Phylum", "Bremen", "Brake", "Bremerhaven", "Helgoland (T)", "Helgoland (S)")
phyla_tw
dim(phyla_tw)

#change to long format
phyla_tw_melt <- melt(phyla_tw)
phyla_tw_melt$Phylum <- factor(phyla_tw_melt$Phylum, levels = phyla_tw$Phylum) 

#define variables
phyla_tw_melt$sites <- plyr::mapvalues(phyla_tw_melt$variable, 
                                 c("Bremen", "Brake", "Bremerhaven", "Helgoland (T)", "Helgoland (S)"), 
                                 c("Bremen", "Brake", "Bremerhaven", "Helgoland (T)", "Helgoland (S)"))
phyla_tw_melt

####wood
phyla_wood <- dna_phyla_relabund[c("phylum", "bremen.wood", "brake.wood", "bremerhaven.wood", "helgolandT.wood", "helgolandS.wood")]
colnames(phyla_wood) <- c("Phylum", "Bremen", "Brake", "Bremerhaven", "Helgoland (T)", "Helgoland (S)")
phyla_wood
dim(phyla_wood)

#change to long format
phyla_wood_melt <- melt(phyla_wood)
phyla_wood_melt$Phylum <- factor(phyla_wood_melt$Phylum, levels = phyla_wood$Phylum) 

#define variables
phyla_wood_melt$sites <- plyr::mapvalues(phyla_wood_melt$variable, 
                                   c("Bremen", "Brake", "Bremerhaven", "Helgoland (T)", "Helgoland (S)"), 
                                   c("Bremen", "Brake", "Bremerhaven", "Helgoland (T)", "Helgoland (S)"))
phyla_wood_melt

####water 3
phyla_water3 <- dna_phyla_relabund[c("phylum", "bremen.water3", "brake.water3", "bremerhaven.water3", "helgolandT.water3", "helgolandS.water3")]
colnames(phyla_water3) <- c("Phylum", "Bremen", "Brake", "Bremerhaven", "Helgoland (T)", "Helgoland (S)")
phyla_water3
dim(phyla_water3)

#change to long format
phyla_water3_melt <- melt(phyla_water3)
phyla_water3_melt$Phylum <- factor(phyla_water3_melt$Phylum, levels = phyla_water3$Phylum) 

#define variables
phyla_water3_melt$sites <- plyr::mapvalues(phyla_water3_melt$variable, 
                                     c("Bremen", "Brake", "Bremerhaven", "Helgoland (T)", "Helgoland (S)"), 
                                     c("Bremen", "Brake", "Bremerhaven", "Helgoland (T)", "Helgoland (S)"))
phyla_water3_melt

####water 02
phyla_water02 <- dna_phyla_relabund[c("phylum", "bremen.water02", "brake.water02", "bremerhaven.water02", "helgolandT.water02", "helgolandS.water02")]
colnames(phyla_water02) <- c("Phylum", "Bremen", "Brake", "Bremerhaven", "Helgoland (T)", "Helgoland (S)")
phyla_water02
dim(phyla_water02)

#change to long format
phyla_water02_melt <- melt(phyla_water02)
phyla_water02_melt$Phylum <- factor(phyla_water02_melt$Phylum, levels = phyla_water02$Phylum) 

#define variables
phyla_water02_melt$sites <- plyr::mapvalues(phyla_water02_melt$variable, 
                                      c("Bremen", "Brake", "Bremerhaven", "Helgoland (T)", "Helgoland (S)"), 
                                      c("Bremen", "Brake", "Bremerhaven", "Helgoland (T)", "Helgoland (S)"))
phyla_water02_melt


# visualization: phyla bar plots
...
(phyla_hdpe_plot <- ggplot(phyla_hdpe_melt, aes(x = sites, y = value)) + 
  geom_bar(aes(fill = Phylum), color = "#FFFFFF", position = position_stack(), stat = "identity") +
...
(phyla_tw_plot <- ggplot(phyla_tw_melt, aes(x = sites, y = value)) + 
  geom_bar(aes(fill = Phylum), color = "#FFFFFF", position = position_stack(), stat = "identity") +
...
(phyla_wood_plot <- ggplot(phyla_wood_melt, aes(x = sites, y = value)) + 
  geom_bar(aes(fill = Phylum), color = "#FFFFFF", position = position_stack(), stat = "identity") +
  theme(plot.margin = margin(t = 1, r = -2, b = 1, l = -2, unit = "cm"),
        axis.title.y = element_blank(),
...
(phyla_water3_plot <- ggplot(phyla_water3_melt, aes(x = sites, y = value)) + 
  geom_bar(aes(fill = Phylum), color = "#FFFFFF", position = position_stack(), stat = "identity") +
  theme(plot.margin = margin(t = 1, r = -2, b = 1, l = -2, unit = "cm"),
        axis.title.y = element_blank(),
        axis.title.x = element_blank()) +
...
(phyla_water02_plot <- ggplot(phyla_water02_melt, aes(x = sites, y = value)) + 
  geom_bar(aes(fill = Phylum), color = "#FFFFFF", position = position_stack(), stat = "identity") +
...
# combine plots
(phyla_bar <- ggarrange(phyla_hdpe_plot, phyla_tw_plot, phyla_wood_plot, phyla_water3_plot, phyla_water02_plot, 
                       common.legend = T, legend = "right", nrow = 1, ncol = 5, align = "hv"))

ggsave("phyla_bar_plot.tiff", phyla_bar, width = 500, height = 285, units = "mm", dpi = 300)
